...
class LoggingSamplerV2(BaseSamplerV2):
    ...
    def __init__(self, inner, label: str, log_every: int = 1, max_retries: int = 1,
        ...
        # per-job metadata for post-hoc charts (timeline, cumulative circuits, …)
    ...
    def run(self, pubs, *, shots=None):
        ...
        # capture per-job metadata (used later by the timeline / cumulative charts)
        try:
            _depth = circ.depth() if circ is not None else 0
            _qubits = circ.num_qubits if circ is not None else 0
        except Exception:
            _depth = _qubits = 0
        self.job_records.append({
            "idx": self.job_count,
            "t": time.time() - self._t0,      # seconds since first job
            "n_circuits": n_pubs,
            "cumulative_circuits": self.pub_count,
            "depth": _depth,
            "qubits": _qubits,
        })
